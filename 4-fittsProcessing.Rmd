...
vs <- read.csv("./visualSearchTimes.csv")
...
the distance * condition interaction is not clear
```{r interaction deep dive}

d %>%
  ggplot(aes(x=rt, y=diffInd) ) +
  geom_bin2d() +
  theme_bw()+
  facet_grid(~visCond)

d %>%
  ggplot(aes(x = rt, fill = as.factor(diffInd)))+
...
d %>%
  mutate(diffInd = as.factor(diffInd))%>%
  group_by(visCond, diffInd) %>%
  summarise(se = sd(rt) / sqrt(length(rt)),
            rt = mean(rt))%>%
  ggplot(aes(x = diffInd, y = rt))+
  geom_col(aes(fill = visCond), position = position_dodge(0.8), width = 0.7)+
  geom_errorbar(
    aes(ymin = rt-se, ymax = rt+se, group = visCond),
    width = 0.2, position = position_dodge(0.8)
    )+
  scale_fill_manual(values = c("grey80", "grey30"))+
  xlab("Index of difficulty")+
...
ggplot(fortify(gammaFinal), aes(x = rt, y = diffInd, color= visCond)) +
...
```{r model maximisation comparison}

gammaMin <- glmer(rt ~ visSearch.s + visCondNum + diffInd.s + visCondNum*diffInd.s + #FIXED
                   (1|id), # RANDOM
              family = Gamma(link = "identity"),
              data = d, 
              control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 10000000)))
...
gammaTwoA <- glmer(rt ~ visSearch.s + visCondNum + diffInd.s + visCondNum*diffInd.s + #FIXED
                   (diffInd.s|id), # RANDOM
              family = Gamma(link = "identity"),
              data = d, 
              control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 10000000)))

gammaThree  <- glmer(rt ~ visSearch.s + visCondNum + diffInd.s + visCondNum*diffInd.s + #FIXED
                   (visCondNum|id) + (diffInd.s|id), # RANDOM
              family = Gamma(link = "identity"),
              data = d, 
              control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 10000000)))
...
modelsummary(list("Intercept only" = gammaMin, 
                  "Visual condition slope only" = gammaTwo,
                  "Index of Difficulty slope only" = gammaTwoA,
                  "Visual condition and index of difficuly slopes" = gammaThree,
                  "Visual condition and index of difficuly slopes with correlations" = gammaFinal),
             stars = T,
             title = "Clicking task model outpus",
             estimate = "{estimate} [{conf.low}, {conf.high}]{stars}",
             statistic = "({std.error})",
